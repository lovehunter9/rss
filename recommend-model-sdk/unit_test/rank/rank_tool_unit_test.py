...
class RankToolUnitTest(unittest.TestCase):
    ...
    def test_rank(self):
        ...
        for current_url,current_embeddings in  url_to_embeddings.items():
            ...
            base_id_to_embedding[current_url] = embedding_array
            embedding_array_list.append(embedding_array)
            print(url_to_articles[current_url]["created_at"])
            samples_tuple.append((current_url,{
                "embedding":embedding_array,
                "created_at":datetime.fromtimestamp(url_to_articles[current_url]["created_at"]*1.0/1000)
                                               }))
        ...
        for current_tuple in query_sample_tuple:
            current_embedding_info = current_tuple[1]
            current_embedding_info["last_reviewed"] = datetime.now() - timedelta(hours=4)
            query_url_to_embedding_dict[current_tuple[0]] = current_embedding_info
        rank_tool = RankTool(base_url_to_embedding_dict)
        result = rank_tool.rank(query_url_to_embedding_dict,100)
        print(result)
        result = rank_tool.rank(dict(),100)
        print(result)
