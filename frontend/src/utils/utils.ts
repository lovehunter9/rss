...
export const formatContentHtml = (htmlContent: string) => {
  ...
  const processedHtml1 = htmlContent.replace(/<p(.*?)>/gi, '<p$1 style="margin: 0.5em 0;">');
  // Process <img> tags
  const processedHtml2 = processedHtml1.replace(/<img(.*?)width=["']([^"']*)["'](.*?)>/gi, '<img$1style="max-width:100%;height:auto;" width="$2"$3>');
  // Process <br> tags
  const newContent = processedHtml2.replace(/<br\/?>/gi, '<br style="clear:both;">');
